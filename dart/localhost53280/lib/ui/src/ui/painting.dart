...
class ImageFilter {
  ...
  factory ImageFilter.matrix(Float64List matrix4,
    ...
    if (engine.useCanvasKit) {
      return engine.CkImageFilter.matrix(
          matrix: matrix4, filterQuality: filterQuality);
    }
    // TODO(yjbanov): implement FilterQuality.
    return engine.EngineImageFilter.matrix(
        matrix: matrix4, filterQuality: filterQuality);
  }
...
Future<Codec> webOnlyInstantiateImageCodecFromUrl(Uri uri,
  ...
  if (engine.useCanvasKit) {
    return engine.skiaInstantiateWebImageCodec(uri.toString(), chunkCallback);
  } else {
...
void decodeImageFromPixels(
  ...
  _createBmp(pixels, width, height, rowBytes ?? width, format)
