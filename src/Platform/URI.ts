...
abstract class Uri {
  ...
  abstract get hasAuthority () : boolean
...
class _Uri implements Uri {
  ...
  static _Uri _makeHttpUri(String scheme, String? authority,
    ...
    if (authority != null && authority.isNotEmpty) {
      ...
      // Split host and port.
      for (; hostEnd < authority.length; hostEnd++) {
        if (authority.codeUnitAt(hostEnd) == kColon) {
  ...
  static void _checkWindowsDriveLetter(int charCode, bool argumentError) {
    if ((kUpperCaseA <= charCode && charCode <= kUpperCaseZ) ||
  ...
  static List<String> _computePathSegments(String pathToSplit) {
    if (pathToSplit.isNotEmpty && pathToSplit.codeUnitAt(0) == kSlash) {
