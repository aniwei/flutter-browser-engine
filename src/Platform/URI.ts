...
export class URI {
  ...
  static computePathSegments (pathToSplit: string): string[] {
    ...
    return pathToSplit.isEmpty ? 
      [] : List<String>.unmodifiable(pathToSplit.split('/').map(URI.decodeComponent))
  }
  ...
  static Map<String, List<String>> _computeQueryParametersAll(String? query) {
    ...
    queryParameterLists.updateAll(toUnmodifiableStringList);
  ...
  static makeWindowsFileURL (
    ...
    if (
      ...
      _checkWindowsDriveLetter(path.codeAt(0), true)
    ...
    if (path.startsWith('\\')) {
      if (path.startsWith('\\', 1)) {
        ...
        const hostPart: string = (pathStart < 0) ? 
          path.substring(2) : path.substring(2, pathStart)

        const pathPart: string = (pathStart < 0) ? 
  ...
  replace ({ 
    ...
    if (
      ...
      if (
        (
          isFile || 
          (hasAuthority && !currentPath.isEmpty)
        ) &&
    ...
    if (
      ...
      query = makeQuery(query, 0, stringOrNullLength(query), queryParameters)
    } else {
      query = this.query
    }
  ...
  static String? _normalizeEscape(String source, int index, bool lowerCase) {
    assert(source.codeAt(index) == kPercent);
    if (index + 2 >= source.length) {
      return "%"; 
    }
    int firstDigit = source.codeAt(index + 1);
    int secondDigit = source.codeAt(index + 2);
  ...
  static String _escapeChar(int char) {
    ...
    if (char < 0x80) {
      ...
      codeUnits[1] = kHexDigits.codeAt(char >> 4);
      codeUnits[2] = kHexDigits.codeAt(char & 0xf);
    } else {
      ...
      while (--encodedBytes >= 0) {
        ...
        codeUnits[index + 1] = kHexDigits.codeAt(byte >> 4);
        codeUnits[index + 2] = kHexDigits.codeAt(byte & 0xf);
        index += 3;
        flag = 0x80; 
      }
    }
  ...
  static String? _normalize(
    ...
    while (index < end) {
      int char = component.codeAt(index);
      if (char < 127 && (charTable[char >> 4] & (1 << (char & 0x0f))) != 0) {
        ...
        if (char == kPercent) {
          ...
          if ((char & 0xFC00) == 0xD800) {
            ...
            if (index + 1 < end) {
              int tail = component.codeAt(index + 1);
              if ((tail & 0xFC00) == 0xDC00) {
  ...
  static String _escapeScheme(String path) {
    if (path.length >= 2 && _isAlphabeticCharacter(path.codeAt(0))) {
      for (int i = 1; i < path.length; i++) {
        int char = path.codeAt(i);
        if (char == kColon) {
  ...
  static String _toWindowsFilePath(Uri uri) {
    ...
    if (segments.length > 0 &&
      ...
      _checkWindowsDriveLetter(segments[0].codeAt(0), false);
...
export class URIData {
  static noScheme = -1
  static base64: Base64Codec = base64
  ...
  static fromBytes(
    ...
    List<int> indices = [noScheme];
    writeURI(mimeType, null, parameters, buffer, indices);
  ...
  static fromString (content: string, {
    ...
    encoding ??= ascii;
    writeURI(mimeType, encodingName, parameters, buffer, indices);
    indices.add(buffer.length);
    if (base64) {
      buffer.write(';base64,');
      indices.add(buffer.length - 1);
  ...
  static void writeURI(
    ...
    if (mimeType.isEmpty || identical(mimeType, "application/octet-stream")) {
      ...
      int slashIndex = validateMimeType(mimeType);
      if (slashIndex < 0) {
  ...
  static validateMimeType(String mimeType): number {
    int slashIndex = -1;
    for (int i = 0; i < mimeType.length; i++) {
      var char = mimeType.codeAt(i);
  ...
  static UriData parse(String uri) {
    if (uri.length >= 5) {
      int dataDelta = startsWithData(uri, 0);
      if (dataDelta == 0) {
  ...
  Uri _computeUri() {
    ...
    return DataURI(this, path, query);
  }
  ...
  static void _uriEncodeBytes(
    ...
    for (int i = 0; i < bytes.length; i++) {
      ...
      if (byte < 128 &&
        ...
        buffer.writeCharCode(kHexDigits.codeAt(byte >> 4));
        buffer.writeCharCode(kHexDigits.codeAt(byte & 0x0f));
      }
    }
    if ((byteOr & ~0xFF) != 0) {
      for (int i = 0; i < bytes.length; i++) {
  ...
  String toString() =>
      (_separatorIndices[0] == noScheme) ? "data:$_text" : _text;
...
List<Uint8List> _createTables() {
  ...
  void setRange(Uint8List target, String range, int transition) {
    for (int i = range.codeAt(0), n = range.codeAt(1); i <= n; i++) {
...
class _SimpleUri implements Uri {
  ...
  Uri replace(
    ...
    if (path != null || pathSegments != null) {
      path = _Uri.makePath(path, 0, stringOrNullLength(path), pathSegments,
          scheme, hasAuthority);
    } else {
    ...
    if (query != null || queryParameters != null) {
      query = _Uri.makeQuery(
          query, 0, stringOrNullLength(query), queryParameters);
  ...
  Uri resolveUri(Uri reference) {
    ...
    return toNonSimple().resolveUri(reference);
  }
  ...
  static int _packageNameEnd(_SimpleUri uri) {
    if (uri._isPackage && !uri.hasAuthority) {
      ...
      return skipPackageNameChars(uri._uri, uri._pathStart, uri._queryStart);
    }
    return -1;
  }
  ...
  Uri _simpleMerge(_SimpleUri base, _SimpleUri ref) {
    ...
    if (ref.hasAuthority) {
      ...
      if (isSimple) {
        ...
        return toNonSimple().resolveUri(ref);
      }
    }
    if (ref.hasEmptyPath) {
      if (ref.hasQuery) {
    ...
    while (baseEnd > baseStart) {
      baseEnd--;
      int char = baseUri.codeAt(baseEnd);
      if (char == kSlash) {
        insert = "/";
  ...
  bool operator ==(Object other) {
   ...
   toNonSimple (): URI {
    return new URI({
      scheme: this.scheme,
      userInfo: this.userInfo,
      hasAuthority: this.hasAuthority ? this.host : null,
      hasPort: this.hasPort ? this.port : null,
      path: this.path,
      hasQuery: this.hasQuery ? this.query : null,
      hasFragment: this.hasFragment ? this.fragment : null
    })
  }
...
function startsWithData (
  ...
  const delta = (text.codeAt(start + 4) ^ kColon) * 3
  delta |= text.codeAt(start) ^ 0x64 /*d*/
  delta |= text.codeAt(start + 1) ^ 0x61 /*a*/
  delta |= text.codeAt(start + 2) ^ 0x74 /*t*/
  delta |= text.codeAt(start + 3) ^ 0x61 /*a*/
  return delta
}


function stringOrNullLength (s: string | null): number {
...
function skipPackageNameChars(
  ...
  for (let i = start; i < end; i++) {
    const char = source.codeAt(i)

    if (char === kSlash) {
      return dots !== 0 ? i : -1
    }
    
    if (char === kPercent || char == kColon) {
      return -1
    }
